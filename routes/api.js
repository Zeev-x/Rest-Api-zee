...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6283898698875 , key nya gratis'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6285755007597 , key nya gratis'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'zeegans') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'zeegans') return res.json(loghandler.invalidKey)
...
router.get('/nulis', async (req, res, next) => {
    ...
    if (!text) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter text"})
       ...
       fetch(encodeURI(`https://api.zeks.xyz/api/nulis?text=${text}&apikey=apivinz`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
